import Link from 'next/link';

export default function Footer() {
  return (
    <footer className="bg-ink border-t border-ember/20 pb-24 md:pb-0">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
          {/* Brand */}
          <div className="md:col-span-1">
            <Link href="/" className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 bg-ember flex items-center justify-center flex-shrink-0">
                <svg viewBox="0 0 40 40" className="w-6 h-6 text-charcoal" fill="currentColor">
                  <path d="M20 4C11.163 4 4 11.163 4 20s7.163 16 16 16 16-7.163 16-16S28.837 4 20 4zm0 2c7.732 0 14 6.268 14 14s-6.268 14-14 14S6 27.732 6 20 12.268 6 20 6z"/>
                  <path d="M14 15c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm8 0c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm-7 7c0 2.76 2.24 5 5 5s5-2.24 5-5H15z"/>
                </svg>
              </div>
              <div>
                <div className="font-oswald text-washi text-base font-bold tracking-wider">BONDI CAFE</div>
                <div className="font-open-sans text-ember text-xs tracking-widest uppercase">Bondi Beach</div>
              </div>
            </Link>
            <p className="font-open-sans text-washi/60 text-sm leading-relaxed">
              Specialty coffee, all-day breakfast, and fresh seasonal food — right on the doorstep of Bondi Beach.
            </p>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="font-oswald text-ember text-sm tracking-widest uppercase mb-4">Navigate</h3>
            <ul className="space-y-2">
              {[
                { href: '/', label: 'Home' },
                { href: '/menu', label: 'Our Menu' },
                { href: '/experience', label: 'Our Story' },
                { href: '/contact', label: 'Contact' },
              ].map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="font-open-sans text-washi/60 text-sm hover:text-ember transition-colors">
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact */}
          <div>
            <h3 className="font-oswald text-ember text-sm tracking-widest uppercase mb-4">Visit Us</h3>
            <ul className="space-y-2 font-open-sans text-washi/60 text-sm">
              <li>
                <a
                  href="https://maps.google.com/?q=94+Campbell+Parade+Bondi+Beach+NSW+2026"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-ember transition-colors"
                >
                  94 Campbell Parade<br />Bondi Beach NSW 2026
                </a>
              </li>
              <li className="mt-3">
                <span className="font-oswald text-ember text-xs tracking-widest uppercase">Order Online</span>
              </li>
              <li>
                <a
                  href="https://www.ubereats.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-ember transition-colors"
                >
                  Uber Eats
                </a>
              </li>
              <li>
                <a
                  href="https://www.doordash.com"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="hover:text-ember transition-colors"
                >
                  DoorDash
                </a>
              </li>
            </ul>
          </div>

          {/* Hours */}
          <div>
            <h3 className="font-oswald text-ember text-sm tracking-widest uppercase mb-4">Opening Hours</h3>
            <ul className="space-y-1 font-open-sans text-washi/60 text-sm">
              <li className="flex justify-between gap-4">
                <span>Every Day</span>
                <span>5:00 AM – 11:00 PM</span>
              </li>
            </ul>
            <p className="font-open-sans text-washi/40 text-xs mt-3">Kitchen closes at 3:00 PM · Bar from 5:00 PM</p>
            <div className="mt-6 flex gap-3">
              <a
                href="https://www.instagram.com"
                target="_blank"
                rel="noopener noreferrer"
                className="w-8 h-8 rounded-full border border-ember/30 flex items-center justify-center text-ember hover:bg-ember hover:text-charcoal transition-all duration-200"
                aria-label="Instagram"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/>
                </svg>
              </a>
              <a
                href="https://www.facebook.com"
                target="_blank"
                rel="noopener noreferrer"
                className="w-8 h-8 rounded-full border border-ember/30 flex items-center justify-center text-ember hover:bg-ember hover:text-charcoal transition-all duration-200"
                aria-label="Facebook"
              >
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
            </div>
          </div>
        </div>

        <div className="section-divider mt-10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-2 font-open-sans text-washi/30 text-xs">
          <span>© {new Date().getFullYear()} Bondi Cafe. All rights reserved.</span>
          <span>
            Website by{' '}
            <a
              href="https://nikamarketing.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-ember transition-colors"
            >
              Nika Marketing
            </a>
          </span>
        </div>
      </div>
    </footer>
  );
}
